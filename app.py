...
DEFAULTS = {
    ...
    "max_age_days": "90", "daily_limit": "30", "interval_seconds": "90",
...
def page(title, body, request=None):
    ...
    nav = "<nav><b>OMTOGEL Follow-up</b><a href='/'>Dashboard</a><a href='/contacts'>Kontak</a><a href='/settings'>Pengaturan</a><a href='/telegram'>Telegram</a><a href='/logout'>Keluar</a></nav>" if request and authorized(request) else ""
...
async def sync(request: Request):
    ...
    cutoff = now() - timedelta(days=int(setting("max_age_days")))
    total = 0
    async for dialog in client.iter_dialogs():
        ...
        if not inbound or inbound < cutoff: continue
...
async def queue_worker():
    while True:
        try:
            ...
            try:
                await client.send_message(item["user_id"], setting(f"message_{item['step']}"))
                sent = now(); db.execute("UPDATE queue SET status='sent',sent_at=?,error=NULL WHERE id=?", (iso(sent),item["id"]))
